...
export function SectionRow({ section, stickers, onOpen, delay = 0 }: Props) {
  ...
  return (
    ...
    >
      <div
        className="h-12 w-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 border border-white/10"
        style={{
          backgroundImage: `linear-gradient(135deg, ${section.accent}33, ${section.accent}10)`,
        }}
      >
        {section.kind === "team" ? section.flag : section.short.charAt(0)}
      </div>
        ...
        <div className="flex items-center gap-2 mt-1.5">
          <div className="flex-1 h-1.5 rounded-full bg-secondary overflow-hidden">
            <motion.div
              ...
              className={cn("h-full rounded-full", complete ? "bg-[hsl(var(--success))]" : "bg-foreground/85")}
            />
          </div>
